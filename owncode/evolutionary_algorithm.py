...
class evolutionary_algorithm:
    ...
    def evaluate_population(self, population: list[np.ndarray]) -> list[float]:
        ...
        with ctx.Pool(cpu_count) as pool:
            fitness_scores = pool.map(self.eval_func, population)
        return fitness_scores

    def eval_func(self, genotype: np.ndarray) -> Any:
        robot = Robot(genotype)
        robot.graph
        return self.evaluator(robot)

    # --- Population Management --- #
    def create_initial_population(self) -> list[list[np.ndarray]]:
        pop: list[list[np.ndarray]] = []
        for _ in range(self.population_size):
            g = self.random_genotype()
            g = self.ensure_viable_population(generation_idx=0, candidate=g, population=None)
            pop.append(g)
        return pop
    ...
    def run(self) -> tuple[list[np.ndarray], list[float], list[float]]:
        ...
        for generation in range(self.generations):
            ...
            while len(new_population) < self.population_size:
                ...
                # Ensure viability
                child1 = self.ensure_viable_population(generation, child1, population, fitness_scores)
                new_population.append(child1)
                # Add to new population (check size to avoid exceeding population_size)
                
                if len(new_population) < self.population_size:
                    child2 = self.ensure_viable_population(generation, child2, population, fitness_scores)
    ...
    def generate_viable_child(self, parent1: list[np.ndarray], parent2: list[np.ndarray]) -> list[np.ndarray]:
        if RNG.random() < self.crossover_rate:
           child1, child2 = self.crossover(parent1, parent2)
           child = child1 if RNG.random() < 0.5 else child2
        else: 
           base = parent1 if RNG.random() < 0.5 else parent2
           child = [gene.copy() for gene in base]
        return self.mutate(child)

    def ensure_viable_population(self, generation_idx:int, candidate: list[np.ndarray], population: list[list[np.ndarray]], fitness_scores: list[float] | None = None) -> list[np.ndarray]:
        if self.viability_checker is None:
            return candidate
        
        tries = 0
        g = candidate
        while tries < self.viability_attempts_per_slot and not self.viability_checker(g):
            tries += 1
            print(f"  → Generation {generation_idx}: Viability attempt {tries}/{self.viability_attempts_per_slot} failed, generating new child")
            if generation_idx == 0:
                g = self.random_genotype()
            else: 
                assert population is not None and fitness_scores is not None, "Population and fitness_scores must be provided for viability checks after generation 0"
                parent1, parent2 = self.select_parents(population, fitness_scores)
                g = self.generate_viable_child(parent1, parent2)
        return g