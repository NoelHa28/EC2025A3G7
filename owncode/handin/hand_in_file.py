...
def load_cpg_genotype(path: str) -> np.ndarray:
    return np.load(path, allow_pickle=True)

brain = EvolvableCPG(9)
brain.set_genotype(load_cpg_genotype("G7_best_robot_brain.npy"))

def cpg_controller(model, data) -> np.ndarray:
    """
    Step function to be used as the robot's controller.
    """
    return brain.step()